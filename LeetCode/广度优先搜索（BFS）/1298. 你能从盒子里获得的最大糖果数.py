...
class Solution:
    def maxCandies(self, status: List[int], candies: List[int], keys: List[List[int]], containedBoxes: List[List[int]],
        ...
        """
        思路：BFS
        1. 将所有找到的盒子，加入队列中，如果盒子是开着的或者有该盒子的钥匙，则可获得里面的糖果，否则将该盒子放入在一边，即last集合中
        2. 当队列中的元素遍历完后，看看剩余的钥匙能不能打开last集合里面的盒子
        @param status:
        @param candies:
        @param keys:
        ...
        while queue:
            ...
            if containedBoxes[box]:
                for b in containedBoxes[box]:
                    if b not in visitBox:
                        queue.append(b)
            if keys[box]:
                for num in keys[box]:
                    if num not in visitKey:
                        hasKey[num] = True

            if status[box] or hasKey[box]:
                res += candies[box]
                visitKey.add(box)
                hasKey[box] = False
            else:
