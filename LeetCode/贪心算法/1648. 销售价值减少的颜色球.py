...
class Solution:
    def maxProfit1(self, inventory: List[int], orders: int) -> int:
        ...
        mod = 10 ** 9 + 7
        # 二分查找阈值T
        l, r, T = 0, max(inventory), 0
        while l <= r:
            mid = l + (r - l) // 2
            cost = sum([num - mid for num in inventory if num >= mid])
            if cost <= orders:
                T = mid
                r = mid - 1
            else:
        ...
        res = orders - sum([num - T for num in inventory if num >= T])
    ...
    def maxProfit2(self, inventory: List[int], orders: int) -> int:
        heap = []
        nums = collections.Counter(inventory)
        ...
        while heap:
            ...
            if heap:
                nex_k, nex_v = heap[0]
            if orders >= (nex_k - k) * v:
                res += range_sum(-nex_k + 1, -k)
                orders -= (nex_k - k) * v
                heap[0][1] += v

            else:
                a = orders // -k
                res += range_sum(-k - a + 1, -k)
                orders -= a * (-k)
                res += orders * -k
        return res
...
if __name__ == '__main__':
    inventory = [2, 5]
    orders = 4
    print(Solution().maxProfit1(inventory, orders))
    print(Solution().maxProfit2(inventory, orders))
