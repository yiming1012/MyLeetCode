...
class Solution:
    def minDays1(self, n: int) -> int:
        ...
        def dfs(n):
            if n < 2:
                return n
            return min(dfs(n // 2) + n % 2 + 1, dfs(n // 3) + n % 3 + 1)
    ...
    def minDays2(self, n: int) -> int:
        ...
        def dfs(n):
            ...
            if n in memo:
                return memo[n]
            memo[n] = min(dfs(n // 2) + n % 2 + 1, dfs(n // 3) + n % 3 + 1)
            return memo[n]

        return dfs(n)

    def minDays3(self, n: int) -> int:
        """
        思路：BFS
        1. 将每一个状态加到队列中，先结束的即为答案
        @param n:
        @return:
        """
...
if __name__ == '__main__':
    n = 10000
    print(Solution().minDays1(n))
    print(Solution().minDays2(n))
    print(Solution().minDays3(n))
